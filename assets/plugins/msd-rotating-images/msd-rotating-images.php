...
/**
 *
 * Plugin Name: MSD Rotating Images
 * Plugin URI: http://pluginbuddy.com/
 * Description: Custom Edit of DisplayBuddy Series - Rotate images using transitions, such as fade or slide, or static random image on page load.
 * Version: 1.0.32
			...
			if ( ! empty($this->_errors) ) { // Report errors.
				...
				foreach ( (array) $this->_options['image_ids'] as $entry ) {
					...
					if ( wp_attachment_is_image( $id ) ) {
						...
						$data = iThemesFileUtility::resize_image( $file, $this->_options['width']*$sizemult, $this->_options['height']*$sizemult, false );
				...
				if ( ( '1' == $this->_options['enable_fade'] ) && ( count( $files ) > 1 ) ) {
					...
					add_action( 'wp_footer', array( &$this, 'print_scrollable' ) );
				...
				if ( 'bottom' === $this->_options['overlay_text_vertical_position'] )
					...
					$return .= '<div class="scrollable" id="scrollable"><ul class="contest-list items scroller-'.$this->_instanceCount.'">';
					foreach($files AS $file){ 
						...
						if ( ! empty( $file['url'] ) ) {
							$link_start = "					<a href=\"{$file['url']}\" class=\"rotating-images-link_".$this->_instanceCount."\"{$target}>\n";
							$link_end = "					</a>\n";
						}
						$thumb = '<img src="'.$file['image'].'" />';
				    	$contest_list .= '
				     	<li>
				     		<div class="img">
				     		'.$link_start.$thumb.$link_end.'
				     		</div>
							<div class="clear"></div>
						</li>';
