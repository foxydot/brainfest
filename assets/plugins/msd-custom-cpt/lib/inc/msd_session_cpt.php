...
function register_tax_timeslot() {
	...
	function list_sessions( $atts ) {
	    ...
	    foreach($items AS $item){
            ...
            $session_data[$session_info->get_the_value('timeslot')][$session_info->get_the_value('track')]['excerpt'] = $excerpt?msdlab_get_excerpt($item->ID):'';
	     }
		//structure table
		//table header
		$headerrow = '<tr><th></th>';
			...
			$table .= '<tr>
				...
				foreach($tracks AS $track){
					if($session_data[$timeslot->term_id]['all']){
						$table .= '<td colspan="'.$num_tracks.'"><a style="font-weight: 700;" href="'.get_permalink($session_data[$timeslot->term_id]['all']['post']->ID).'">'.$session_data[$timeslot->term_id]['all']['post']->post_title.'</a><br /> '.$session_data[$timeslot->term_id][$track->term_id]['subtitle'].'</td>';
						break 1;
					} else {
						$table .= '<td class="track"><a style="font-weight: 700;" href="'.get_permalink($session_data[$timeslot->term_id][$track->term_id]['post']->ID).'">'.$session_data[$timeslot->term_id][$track->term_id]['post']->post_title.'</a><br /> '.$session_data[$timeslot->term_id][$track->term_id]['subtitle'].'</td>';
					}
				}
	        $table .= '</tr>';
          ...
          if($session_data[$timeslot->term_id]['all']){
            ...
            break 1;
              ...
              if($session_data[$timeslot->term_id][$track->term_id]['post']->post_content == ''){
                $list .= '<li><span>'.$session_data[$timeslot->term_id][$track->term_id]['post']->post_title.'</span><br /> '.$session_data[$timeslot->term_id][$track->term_id]['subtitle'].$session_data[$timeslot->term_id][$track->term_id]['excerpt'].'</li>';
              } else {
                $list .= '<li><a href="'.get_permalink($session_data[$timeslot->term_id][$track->term_id]['post']->ID).'">'.$session_data[$timeslot->term_id][$track->term_id]['post']->post_title.'</a><br /> '.$session_data[$timeslot->term_id][$track->term_id]['subtitle'].$session_data[$timeslot->term_id][$track->term_id]['excerpt'].'</li>';
              }
          }
        }
      $list .= '
		...
		$accordion_display = '<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">'.$list.'</div>';
		return $accordion_display.$style;
	}	
}
